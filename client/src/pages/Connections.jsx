...
const Connections = () => {
    ...
    const dataArray = [
        ...
        {label: 'Following', value: followers, icon: UserCheck},
        {label: 'Pending', value: followers, icon: UserRoundPen},
        {label: 'Connections', value: followers, icon: UserPlus},
    ]
    return (
    <div className='min-h-screen bg-state-50'>
        <div className='max-w-6xl max-auto p-6'>
            ...
            {/* Counts */}
            <div className="mb-8 flex flex-wrap gap-6"> {dataArray.map((item, index) => (
...
{dataArray.find((item) => item.label === currentTab)?.value.map((user) => (
    ...
    <div className="flex-1">
        ...
        {currentTab === 'Following' && (<button className="w-full p-2 text-sm rounded bg-slate-100 
        hover:bg-slate-200 text-black active:scale-95 transition cursor-pointer">Unfollow</button>)}
        {currentTab === 'Pending' && (<button className="w-full p-2 text-sm rounded bg-slate-100 hover:bg-slate-200
        text-black active:scale-95 transition cursor-pointer">Accept</button>)}
        {currentTab === 'Connections' && (<button onClick={()=> navigate(`/messages/${user._id}`)}
        className="w-full p-2 text-sm rounded bg-slate-100 hover:bg-slate-200
        text-black active:scale-95 transition cursor-pointer flex items-center justify-center gap-1">
